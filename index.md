...
|속성|영문 명칭|설명|비고|
|:---:|:---:|:---:|:---:|
|잠금 확인|OpenTable|책상의 서랍이 현재 열 수 있는 상태인지 확인합니다.||

#### 4) 오브젝트 이름 : 서랍 Drawer
|속성|영문 명칭|설명|비고|
|:---:|:---:|:---:|:---:|
|잠금 확인|OpenDrawer|서랍이 현재 열 수 있는 상태인지 확인합니다.||

#### 5) 오브젝트 이름 : 액자 Frame
|속성|영문 명칭|설명|비고|
|:---:|:---:|:---:|:---:|
|낙하 확인|DropCheck|액자와의 상호작용 여부를 저장합니다.||
...
|행동|영문 명칭|설명|
|:---:|:---:|:---:|
|앞으로 이동|Walk_Forward|플레이어 캐릭터를 앞으로 이동한다.|
|뒤로 이동|Walk_Back|플레이어 캐릭터를 뒤로 이동한다.|
|오른쪽으로 이동|Walk_Right|플레이어 캐릭터를 오른쪽으로 이동한다.|
|왼쪽으로 이동|Walk_Left|플레이어 캐릭터를 왼쪽으로 이동한다.|
|상호작용 하기|Interaction|'F'키를 눌러 게임 오브젝트와 상호작용한다.|
|시아 변경|Looking|마우스를 이용하여 시아를 변경한다.|
