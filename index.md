...
|속성|영문 명칭|설명|비고|
|:---:|:---:|:---:|:---:|
|레벨|Level|플레이어 캐릭터의 캐릭터 등급입니다.|레벨이 높을 수록 공격력에 더 높은 추가 수치가 적용됩니다.|
|금화|Gold|게임 내에서 사용되는 재화이자 재화 단위입니다.|게임 내에서 아이템 혹은 장비를 거래할 때 사용됩니다.|
|공격력|AttackPoint|플레이어가 소지한 아이템의 리스트를 저장합니다.||
|이동속도|Speed|플레이어가 클리어한 있는 스테이지를 저장합니다.||
